...
def sidebar():
    ...
    choise = st.sidebar.radio('功能选择',['宏观总结','个人画像','管理员权限'],index = 0)
    if choise == '个人画像':
        ano_choise = st.sidebar.select_slider('请选择一种预警方式', ['学业预警','0','贫困预警'],value = '0')
        
        if ano_choise=='学业预警':
            name = st.sidebar.selectbox('请选择一位学生', df['姓名'].tolist())
            st.progress(100)
            st.spinner()
